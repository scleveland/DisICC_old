...
class DashesController < ApplicationController
  ...
  def index
    ...
    @data_values = Raw::DataValues.all
  ...
  def show
    @data_value = Raw::DataValues.get(params[:id])
  ...
  def create
    @data_value = Raw::DataValues.new(params[:new_data_value])

    respond_to do |format|
      if @data_value.save
        flash[:notice] = 'Raw::DataValues was successfully created.'
  ...
  def update
    ...
    respond_to do |format|
      if @data_value.update_attributes(params[:data_value])
        flash[:notice] = 'Raw::DataValues was successfully updated.'
        format.html { redirect_to(raw_data_value_path(@data_value.id)) }
      else
